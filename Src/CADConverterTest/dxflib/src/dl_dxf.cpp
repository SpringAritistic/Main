...
bool DL_Dxf::processDXFGroup(DL_CreationInterface* creationInterface,
    ...
    // Indicates start of new entity or var
        ...
        strcpy_s(linetype, toString(values[6], "BYLAYER"));
        ...
        if (groupValue[0]=='$') {
            ...
            strcpy_s(settingKey, DL_DXF_MAXLINE, groupValue);
        ...
        if (groupCode<DL_DXF_MAXGROUPCODE) {
            ...
            if (!handled) {
                ...
                strcpy_s(values[groupCode], DL_DXF_MAXLINE, groupValue);
...
void DL_Dxf::writeMText(DL_WriterA& dw,
    ...
    for (i=250; i<length; i+=250) {
        strcpy_s(chunk, 250, &data.text.c_str()[i-250]);
        chunk[250]='\0';
...
int DL_Dxf::getLibVersion(const char* str) {
    ...
    if (idx==3) {
        ...
        strcpy_s(v[0], d[0], str);
        v[0][d[0]] = '\0';

        strcpy_s(v[1], d[1]-d[0]-1, &str[d[0]+1]);
        v[1][d[1]-d[0]-1] = '\0';

        strcpy_s(v[2], d[2]-d[1]-1, &str[d[1]+1]);
        v[2][d[2]-d[1]-1] = '\0';

        strcpy_s(v[3], d[3]-d[2]-1, &str[d[2]+1]);
        v[3][d[3]-d[2]-1] = '\0';
