...
class HM_GlobalTool_EXT x_process_object
{
public:
...
HM_GlobalTool_EXT DWORD do_process_task(const CString& task_name, const x_process_object obj); // 在新进程中执行任务
HM_GlobalTool_EXT bool has_process_task(const CString& task_name); // 当前是否有指定名称的任务
HM_GlobalTool_EXT void force_stop_all_tasks(); // 强行结束所有任务
