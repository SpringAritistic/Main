...
class HM_GlobalTool_EXT console_session_interface
{
public:
...
// 命令行接口，同时支持也支持信息输出
class HM_GlobalTool_EXT console_interface
{
public:
	console_interface();
...
HM_GlobalTool_EXT console_interface* get_console_interface();
HM_GlobalTool_EXT void set_console_interface(console_interface* console_obj);

// 直接以 MT_INFO 类型输出到日志（没有界面也能正常工作）
HM_GlobalTool_EXT void app_print_info(const TCHAR* format, ...);

// 以指定类型直接输出到日志（没有界面也能正常工作）
HM_GlobalTool_EXT void app_print_info(message_type type, const TCHAR* format, ...);

// 以指定类型直接输出到日志（没有界面也能正常工作）
HM_GlobalTool_EXT void app_print_info(output_object out, message_type type, const TCHAR* format, ...);
