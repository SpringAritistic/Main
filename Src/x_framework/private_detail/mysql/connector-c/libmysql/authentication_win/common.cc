...
#endif
{
  ...
  success= (TRUE == LookupAccountNameW(NULL, account_name, NULL, &sid_size,
  ...
  success= (TRUE == LookupAccountNameW(NULL, account_name,
...
#endif
{
  ...
  success = (TRUE == GetTokenInformation(token, TokenUser, NULL, 0, &req_size));
  if (!success && GetLastError() != ERROR_INSUFFICIENT_BUFFER)
  {
#ifndef DBUG_OFF
    Error_message_buf error_buf;
  ...
  success= (TRUE == GetTokenInformation(token, TokenUser, m_data, req_size, &req_size));
...
const char* Sid::as_string()
{
  ...
  if (!m_as_string)
  {
    bool success= (TRUE == ConvertSidToStringSid(m_data->User.Sid, &m_as_string));
...
bool Sid::operator ==(const Sid &other)
{
  ...
  return (TRUE == EqualSid(m_data->User.Sid, other.m_data->User.Sid));
}
