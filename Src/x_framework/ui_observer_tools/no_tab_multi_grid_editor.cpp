...
#include "no_tab_multi_grid_editor.h"
...
//////////////////////////////////////////////////////////////////////////
IMPLEMENT_SERIAL(no_tab_multi_gird_editor, x_mvc_editor_base, 1)
BEGIN_MESSAGE_MAP(no_tab_multi_gird_editor, x_mvc_editor_base)
	ON_WM_CREATE()
	ON_WM_SIZE()
...
no_tab_multi_gird_editor::no_tab_multi_gird_editor()
{
...
bool no_tab_multi_gird_editor::load_data_to_ui(const CString& data_tag)
{
	...
	no_tab_multi_grid_observer* cur_ob = dynamic_cast<no_tab_multi_grid_observer*>(obs);
	if (!cur_ob) return false;

	std::pair<CString, std::vector<CString>> all_tables;
	cur_ob->get_table_template_names(all_tables);

	m_created_grid_view->assign_all_excels(all_tables.second);

	cur_ob->load_data_to_ui(m_data_base);

	for (auto &x : m_data_base)
	{
...
bool no_tab_multi_gird_editor::save_data_from_ui()
{
	x_observer_base* obs = get_ui_observer();
	if (!obs) return false;
...
void no_tab_multi_gird_editor::on_project_data_notify(const CString& node_tag, notifer_action ac)
{
	x_observer_base* obs = get_ui_observer();
...
int no_tab_multi_gird_editor::OnCreate(LPCREATESTRUCT lpCreateStruct)
{
	...
	m_created_grid_view = new multi_grid_view();
	m_created_grid_view->create_direct(this,112,CRect(),false);
...
void no_tab_multi_gird_editor::OnSize(UINT type, int cx, int cy)
{
...
BOOL no_tab_multi_gird_editor::on_cell_value_changed(UINT id, NMHDR * notify_struct, LRESULT * result)
{
	if (!notify_struct) return FALSE;
	NM_THGRID* nm_msg = (NM_THGRID*)notify_struct;

	if (!m_created_grid_view) return FALSE;
	CTHDataBase* db_clicked = m_created_grid_view->find_data_base(id);
...
BOOL no_tab_multi_gird_editor::on_cell_button_clicked(UINT id, NMHDR * notify_struct, LRESULT * result)
{
	if (!notify_struct) return FALSE;
	NM_THGRID* nm_msg = (NM_THGRID*)notify_struct;
...
BOOL no_tab_multi_gird_editor::on_cell_focus_changed(UINT id, NMHDR * notify_struct, LRESULT * result)
{
	if (!notify_struct) return FALSE;
	NM_THGRID* nm_msg = (NM_THGRID*)notify_struct;
...
{
	active_workspace_mvc_editor(data_node_tag, ui_observer_class_name, _T("no_tab_multi_gird_editor"));
}
