...
// 数据库的一个字段信息
class X_FRAMEWORK_API db_field
{
public:
	// 数据转换规则：
...
// 表示数据库中的的一条记录（即一行数据）
class X_FRAMEWORK_API db_record
{
public:
	db_record();
	~db_record();
...
class X_FRAMEWORK_API db_adapter_interface
{
public:
...
//   强烈再考虑支持，此处特作注释以记之。
X_FRAMEWORK_API void regist_database_adapter(db_adapter_interface* adapter);

// 返回注册的数据库操作实例，如果没有注册过则返回内部默认生成的实例（默认实例什么也不做，只保证存在一个实例而已）
// 此接口支持并发访问
X_FRAMEWORK_API db_adapter_interface* get_database_adapter();
