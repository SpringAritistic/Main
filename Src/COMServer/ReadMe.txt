========================================================================
    活动模板库：COMServer 项目概述
========================================================================

应用程序向导已为您创建了此 COMServer 项目，作为编写动态链接库 (DLL) 的起点。
...
COMServer.idl
    ...
    此文件将由 MIDL 编译器处理，用于生成：
        C++ 接口定义和 GUID 声明 (COMServer.h)
        GUID 定义                                (COMServer_i.c)
        类型库                                  (COMServer.tlb)
        封送处理代码                                 （COMServer_p.c 和 dlldata.c）

COMServer.h
    此文件包含 COMServer.idl 中定义的项目的 C++ 接口定义和 GUID 声明。它将在编译过程中由 MIDL 重新生成。
...
StdAfx.h, StdAfx.cpp
    这些文件用于生成名为 COMServer.pch 的预编译头 (PCH) 文件和名为 StdAfx.obj 的预编译类型文件。
...
COMServerps.vcxproj
	...
	此过程生成 dlldata.c、COMServer_i.c 和 COMServer_p.c，这些是生成代理/存根 (stub) DLL 所必需的。
